...
Usage: python -m FallonPrototype.shared.ingest_market_defaults
"""
...
def _find_defaults_path() -> str | None:
    """Find the market defaults JSON file."""
    for path in _DEFAULTS_PATHS:
...
def ingest_market_defaults() -> dict:
    ...
    if not defaults_path:
        print(f"[ingest_market_defaults] No market_defaults.json found")
        return {"records": 0}

    print(f"[ingest_market_defaults] Using: {defaults_path}")
